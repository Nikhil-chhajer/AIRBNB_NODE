...
func ProxyServiceToHotel(targetBaseUrl string, pathPrefix string) http.HandlerFunc {
	target, err := url.Parse(targetBaseUrl) //parse the url and create go based url object out of it
	if err != nil {
		fmt.Println("Error parsing target URL :", err)
		return nil
...
func ProxyServiceToBooking(targetBaseUrl string, pathPrefix string) http.HandlerFunc {
	target, err := url.Parse(targetBaseUrl) //parse the url and create go based url object out of it
	if err != nil {
		fmt.Println("Error parsing target URL :", err)
		return nil
