...
export async function generateRooms(jobData: RoomGenerationJob ) {
    ...
    if (!roomCategory) {
        throw new InternalServerError(`Room category with id ${jobData.roomCategoryId} not found`);
    }
    ...
    if (startDate >= endDate) {
        ...
        throw new InternalServerError(`Start date must be before end date`);
    }

    if (startDate < new Date()) {
       
        throw new InternalServerError(`Start date must be in the future`);
    }
